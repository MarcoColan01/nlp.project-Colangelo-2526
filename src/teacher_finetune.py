...
def build_teacher_model(cfg: TeacherModelConfig):
    ...
    try:
        model = AutoModelForSequenceClassification.from_pretrained(cfg.model_name, config=config, attn_implementation=cfg.attn_implementation,)
    except TypeError:
...
def freeze_bert_layers(model: nn.Module, freeze_embeddings: bool = True, freeze_layers: int = 0) -> None:
    ...
    '''
    Freeze embeddings and the first N layers of BERT encoder.
    '''
    ...
    if freeze_layers > 0 and hasattr(model, "bert") and hasattr(model.bert, "encoder"):
        ...
        print(f"First {freeze_layers}/{total_layers} layers of encoder have been frozen.")

    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Trainable parameters: {trainable_params/1e6:.1f}M / {total_params/1e6:.1f}M ({(trainable_params/total_params)*100:.1f}%)")
...
def load_chunks_dataset(
    ...
    if add_sample_weight:
        if chunk_counts_parquet is None:
            raise ValueError("No chunk_counts_parquet")
